...
import { getDashboardShareState, toggleDashboardShare } from '@/app/actions/dashboards'
...
export default function SidebarPulsoShare() {
  ...
  // Fetch share state on first open
  useEffect(() => {
    if (open && !fetched.current) {
      fetched.current = true
      setFetching(true)
      getDashboardShareState(DASHBOARD_ID)
        .then((state) => {
          setIsPublic(state.is_public)
          setPublicToken(state.public_token)
          setExpiresAt(state.public_token_expires_at)
        })
        .catch(console.error)
  ...
  async function handleToggle() {
    ...
    try {
      ...
      setExpiresAt(result.public_token_expires_at)
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
  ...
  const shareUrl = publicToken ? `${typeof window !== 'undefined' ? window.location.origin : ''}/share/${publicToken}` : ''
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-flex', marginLeft: 'auto' }}>
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--taller-green)' }}
      ...
      {open && (
        ...
        >
          ...
          {fetching ? (
            <p style={{ fontSize: 12, color: 'var(--taller-muted)', margin: 0 }}>Cargando...</p>
          ) : (
            <>
              {/* Toggle */}
