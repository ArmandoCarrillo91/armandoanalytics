...
async function getTenantClientPublic() {
...
export async function getClasesAnioPublic() {
  const client = await getTenantClientPublic()
  const { data } = await client.rpc('execute_query', { query: `
    SELECT
