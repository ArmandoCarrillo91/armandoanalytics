        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {ocupacion.map((row, i) => {
                ...
                const hour = new Date(row.class_date).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })
                return (
                  <div key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: TEXT, marginBottom: 2 }}>
                      <span style={mono}>{hour}</span>
                      <span style={{ ...mono, color: MUTED }}>{Number(row.spots_ocupados)}/{totalBicis}</span>
                    </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr 1fr', gap: 12 }}>
            {[
              ...
              <div key={k.label} style={{ background: '#111', borderRadius: 6, padding: 14 }}>
