...
export async function getOcupacionHoy() {
  const { rows } = await pool.query(`
    ...
    JOIN class_schedules_schedule_lnk cssl ON cs.id = cssl.class_schedule_id
    JOIN schedules s ON cssl.schedule_id = s.id
    LEFT JOIN class_reservations cr ON cr.class_date::date = cs.schedule_date::date
      AND EXTRACT(HOUR FROM cr.class_date) = EXTRACT(HOUR FROM s.start_time::time)
      AND (cr.is_refunded = false OR cr.is_refunded IS NULL)
    WHERE cs.schedule_date::date = CURRENT_DATE
    GROUP BY cs.schedule_date, s.start_time, s.end_time
...
export async function getIngresosAcumulados() {
  const { rows } = await pool.query(`
    SELECT
      dia,
      diario,
      SUM(diario) OVER (ORDER BY dia) as acumulado
    FROM (
      SELECT
        DATE_PART('day', created_at) as dia,
        SUM(price) as diario
